...
pub enum ProviderClient {
    ...
    Gemini(crate::providers::gemini::GeminiClient),
}
    ...
    pub fn from_model_with_default_auth(
        ...
        match providers::detect_provider_kind(&resolved_model) {
            ...
            ProviderKind::Xai | ProviderKind::OpenAi => {
                let (provider_name, api_key_env, base_url_env, default_base_url) = if let Some(meta) = metadata {
                    (meta.provider.as_str(), meta.auth_env, meta.base_url_env, meta.default_base_url)
                } else if providers::detect_provider_kind(&resolved_model) == ProviderKind::Xai {
                    ("xAI", "XAI_API_KEY", "XAI_BASE_URL", openai_compat::DEFAULT_XAI_BASE_URL)
                } else {
                    ("OpenAI", "OPENAI_API_KEY", "OPENAI_BASE_URL", openai_compat::DEFAULT_OPENAI_BASE_URL)
                };
                ...
                if providers::detect_provider_kind(&resolved_model) == ProviderKind::Xai {
            ...
            ProviderKind::Gemini => {
                let (api_key_env, _) = if let Some(meta) = metadata {
                    (meta.auth_env, meta.base_url_env)
                } else {
                    ("GEMINI_API_KEY", "GEMINI_BASE_URL")
                };
                Ok(Self::Gemini(crate::providers::gemini::GeminiClient::from_env(
                    api_key_env,
