...
impl OpenAiCompatClient {
    ...
    async fn send_raw_request(
        ...
        let request_url = chat_completions_endpoint(&self.base_url, &self.api_key);
        if request_url.contains("googleapis.com") {
            println!("🚀 [GEMINI DEBUG] Requesting: {}", request_url);
        }
    ...
    match message.role.as_str() {
        "assistant" => {
            ...
            for block in &message.content {
                match block {
                    ...
                    InputContentBlock::Image { .. } => {} // Images not supported in assistant role for OpenAI compat
                }
            }
        ...
        _ => message
            ...
            .filter_map(|block| match block {
                ...
                InputContentBlock::Image { .. } => None, // Images skipped for OpenAI compat (use vision models directly)
            })
            .collect(),
    }
}
...
fn chat_completions_endpoint(base_url: &str, api_key: &str) -> String {
    let trimmed = base_url.trim_end_matches('/');
    let mut url = if trimmed.ends_with("/chat/completions") {
        trimmed.to_string()
    } else {
        format!("{trimmed}/chat/completions")
    };

    if url.contains("googleapis.com") && !url.contains("?key=") {
        url.push_str("?key=");
